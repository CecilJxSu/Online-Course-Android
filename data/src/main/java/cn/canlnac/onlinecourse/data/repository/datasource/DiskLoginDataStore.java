...
/**
 * {@link LoginDataStore} implementation based on file system data store.
 */
class DiskLoginDataStore implements LoginDataStore {

  private final LoginCache loginCache;

  /**
   * Construct a {@link LoginDataStore} based file system data store.
   *
   * @param loginCache A {@link LoginCache} to cache data retrieved from the api.
   */
  DiskLoginDataStore(LoginCache loginCache) {
    this.loginCache = loginCache;
  }

  @Override
  public Observable<LoginEntity> login(String username, String password) {
    return this.loginCache.get(username, password);
  }
}
